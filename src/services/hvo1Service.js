...
export const readDataHvo1 = async (modbusClient, deviceID, deviceLabel) => {
  try {
    const parameters = {
      ...
      'Уровень воды в емкости E1/1': ((await modbusClient.readFloat(deviceID, 0x0008, deviceLabel)) * 16).toFixed(2),
      'Уровень воды в емкости E1/2': ((await modbusClient.readFloat(deviceID, 0x000a, deviceLabel)) * 16).toFixed(2),
